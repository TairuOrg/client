...
export const addItemSchema = (products: Item[], alreadyExistingItem: boolean) =>
  z.object({
    description: z
      .string()
      ...
      .max(
        70,
        "La descripción del artículo no puede tener más de 70 caracteres"
      ),

    barcode_id: z
      .string()
      .min(4, { message: "El código de barras debe tener al menos 4 dígitos" })
      .max(14, {
        message: "El código de barras debe tener como máximo 14 dígitos",
      })
      .regex(/^\d{4,14}$/, {
        message: "El código de barras debe tener entre 4 y 14 dígitos",
      })
      .refine(
        (barcode_id) => {
          if(alreadyExistingItem) return true;
          const exists = products.some(
            (product) => product.barcode_id === barcode_id
          );
          return !exists;
        },
        { message: "Este código ya existe en el sistema" }
      ),
    manufacturer: z
      .string()
    ...
    category: z
      .string()
      .min(1, "La categoría del artículo no puede estar vacía"),
...
export type AddItemSchemaType = z.infer<ReturnType<typeof addItemSchema>>;
